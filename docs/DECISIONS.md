...
**The floor reads the report as text, not through an XML parser.** The first
version of `cobertura_metrics` used `xml.etree.ElementTree.fromstring` with a
`# nosec B314` on it, on the reasoning that the file is generated by coverage.py
one step earlier and is not attacker-controlled. The third reviewer disagreed
before this entry was a day old: the quality gate on the pull request that
introduced it failed on *C Security Rating on New Code*, and the only new
security-shaped construct in the diff was that call. Two scanners naming the
same line is the answer to "is this worth the suppression". Cobertura carries
`filename` on `<class>` and on no other element, so one regex over the text is
what the floors need, and a regex that misread it would name a measured file as
unmeasured — which fails closed. No parser, no suppression, and no `defusedxml`
dependency to harden a file this repository wrote itself.
