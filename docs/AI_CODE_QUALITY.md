...
| **Security** | 2.74× | Static analysis with a scan floor, dependency audit, secret scan. An image scan arrives with the Dockerfile. A route-level actor matrix drives every endpoint as each actor the store distinguishes — anonymous, a member of another case, the four standings — and a second test drives a forged development header in production (`docs/DECISIONS.md` §50) | `bandit` (pinned 3.12 — see §4) + `scan_floors.py`, `pip-audit`, `gitleaks`; `test_the_actor_matrix_for_the_run_view` and `test_production_never_trusts_role_header`; Trivy is owed by `REBUILD_PLAN.md` Phase 7 |
