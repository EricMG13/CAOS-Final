...
def test_a_pack_is_admitted_whole(store: Store) -> None:
    admitted = admit_pack(
        store, case_id="acme", documents=(_document("a" * 64), _document("b" * 64))
    )
...
def test_a_pack_carrying_one_bad_document_admits_none_of_it(store: Store) -> None:
    admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    store.commit()
...
def test_source_set_versions_start_at_one_and_increment(store: Store) -> None:
    admitted = admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    assert pin_source_set(store, case_id="acme", source_ids=admitted) == 1
    assert pin_source_set(store, case_id="acme", source_ids=admitted) == 2
...
def test_versions_are_allocated_under_the_case_lock(
    ...
    def pin(_: int) -> int:
        with psycopg.connect(postgres_dsn, autocommit=False) as connection:
            ...
            return pin_source_set(connection, case_id="acme", source_ids=admitted)
...
def test_a_pinned_source_set_is_immutable(store: Store) -> None:
    admitted = admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    pin_source_set(store, case_id="acme", source_ids=admitted)
    for statement in (
...
def test_a_source_set_naming_a_source_from_another_case_is_refused(
    ...
    mine = admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    theirs = admit_pack(store, case_id="rival", documents=(_document("b" * 64),))
    with pytest.raises(Refusal) as caught:
        pin_source_set(store, case_id="acme", source_ids=(*mine, *theirs))
...
def test_an_empty_source_set_is_refused(store: Store) -> None:
    ...
    admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    with pytest.raises(Refusal) as caught:
        pin_source_set(store, case_id="acme", source_ids=())
...
def test_pinning_the_same_source_twice_pins_it_once(store: Store) -> None:
    (admitted,) = admit_pack(store, case_id="acme", documents=(_document("a" * 64),))
    version = pin_source_set(store, case_id="acme", source_ids=(admitted, admitted))
...
def test_a_document_the_store_refuses_is_a_typed_refusal(store: Store) -> None:
    ...
    with pytest.raises(Refusal) as caught:
        admit_pack(store, case_id="acme", documents=(impossible,))
